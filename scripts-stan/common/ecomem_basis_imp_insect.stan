...
parameters {  
  ...
  real<lower=1.0/15.0, upper=1> tau_outbreak;
...
transformed parameters { 
  ...
  phi_fire = 1/tau_fire;
    ...
    phi_outbreak = 1/tau_outbreak;
  

    g = rep_vector(0, N_years);
    for (year in 1:N_years) {
      for (prior_year in 1:(year-1)){
        if (outbreak[year] == 1){
          g[year] = g[year] + spherical(year-prior_year, phi_outbreak);
        }
      }
      //print(f[year]);
    }
  ...
  for (site in 1:N_sites) {
    //alpha[site] = z[site][(lag+1):N_years] * gamma1 + f[(lag+1):N_years] * gamma2 +
    //  g[(lag+1):N_years] * gamma3;
...
model { 
  ...
  tau_outbreak ~ uniform(1.0/15.0, 1);
  ...
  for (site in 1:N_sites){
    ...
    Y[site][(lag+1):N_years]  ~ normal(alpha[site] + dat[site][(lag+1):N_years,2:(N_covars+1)] * beta + u[site], sigma[site]);
