...
data { 
  ...
  int<lower=0> n_basis;
  int<lower=0> n_knots;
  int<lower=0> include_fire;
  int<lower=0> include_outbreak;
  
  vector[N_years] Y[N_sites];
  matrix[N_years, N_covars] X[N_sites];
  vector[N_years] cmem[N_sites];
  
  vector[N_years] dimem;
  vector[N_years] dfmem;
...
parameters {
  ...
  real<lower=1.0/15.0, upper=1> tau_dimem;
  real gamma2i;
  
  real<lower=1.0/15.0, upper=1> tau_dfmem;
  real gamma2f;
...
transformed parameters { 
  ...
  for (site in 1:N_sites) {
    ...
    phi_dimem = 1/tau_dimem;
    fi = rep_vector(0, N_years);
    for (year in 1:N_years) {
      for (prior_year in 1:(year-1)){
        if (dimem[year] == 1){
          fi[year] = fi[year] + spherical(year-prior_year, phi_dimem);
        }
      }
      //print(f[year]);
    }
    ...
    phi_dfmem = 1/tau_dfmem;
    ff = rep_vector(0, N_years);
    for (year in 1:N_years) {
      for (prior_year in 1:(year-1)){
        if (dfmem[year] == 1){
          ff[year] = ff[year] + spherical(year-prior_year, phi_dfmem);
        }
      }
      //print(f[year]);
    }
  ...
  for (site in 1:N_sites) {
    alpha[site] = gamma0 + z[site][(lag+1):N_years] * gamma1;// + f[(lag+1):N_years] * gamma2;


    alpha[site] = alpha[site] + fi[(lag+1):N_years] * gamma2i;
    alpha[site] = alpha[site] + ff[(lag+1):N_years] * gamma2f;
...
model { 
  ...
  tau_dimem ~ uniform(1.0/15.0, 1);
  tau_dfmem ~ uniform(1.0/15.0, 1); 
  ...
  gamma2i ~ normal(0, 1e5);
  gamma2f ~ normal(0, 1e5);
