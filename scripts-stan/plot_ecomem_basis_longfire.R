...
#######################################################################################
fit = readRDS('scripts-stan/output/fit_ecomem_basis_longfire.RDS')
...
ggplot(data=mem.fire.dat) + 
  ...
  geom_ribbon(aes(x=x, ymin=q5, ymax=q95), fill='dodgerblue', alpha=0.5) +
  theme_bw()
...
beta.quants
coef.dat = rbind(coef.dat, data.frame(par=c('beta1'), t(beta.quants)))
